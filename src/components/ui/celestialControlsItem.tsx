        ...
        <div className={"item" + ((props.sequence.toString() == props.currentSequence.toString())?' active':'')}>
            {props.entity instanceof Celestial && <>
                <div className="header">
                    <div className="name" onClick={() => props.selectEntity(props.sequence)}>
                        {props.entity.name}
                    </div>
                    <div className="buttons">
                        ...
                        {(props.sequence.length > 1) && <button onClick={() => props.deleteEntity(props.sequence)}>
                        ...
                        {(props.sequence.length < 8) && <button onClick={() => props.addEntity(props.sequence)}>
                ...
                {!collapsed && <div className="submenu">
                    {props.entity.satellites.map((e, k) => <CelestialControlsItem key={e.entity.id} entity={e.entity} selectEntity={props.selectEntity} deleteEntity={props.deleteEntity} addEntity={props.addEntity} sequence={[...props.sequence, k]} currentSequence={props.currentSequence}></CelestialControlsItem>)}
                </div>}
            </>}
